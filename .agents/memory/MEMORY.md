...
- [buildspeed-kgp-version-skew](project/buildspeed-kgp-version-skew.md) — `checkPerformance` runs current Spine plugins under BuildSpeed's own pinned KGP; on `NoSuchMethodError` for a KGP API, bump `kotlinVersion` in BuildSpeed's `buildSrc`.
